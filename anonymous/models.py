...
class Anonymous(models.Model):
    ...
    def created_string(self):
        time = timezone.now() - self.registered_date
        if time < timedelta(minutes=1):
            ...
            time_diff = timezone.now().date() - self.registered_date.date()
...
class AnonymousComment(models.Model):
    ...
    def created_string(self):
        ...
        time = timezone.now() - self.created_date
        registered_date = timezone.make_naive(self.registered_date, timezone=timezone.utc)
        time = timezone.now() - registered_date
