...
class UserManager(BaseUserManager):
    ...
    def create_user(self, user_id, password, email, hp, name, student_id, grade, department, circles, auth,
                    **extra_fields):
        if not user_id:
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    groups = models.ManyToManyField(
        ...
        related_name='user_groups',  # 수정된 부분
    ...
    user_permissions = models.ManyToManyField(
        ...
        related_name='user_permissions',  # 수정된 부분
        related_query_name='user',
    )